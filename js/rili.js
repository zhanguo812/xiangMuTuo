$(() => {
  ...
  $(function () {
    $("#days").on("click", "li", function () {
      ...
      var month = $("#calendar-title").html();
      var day = $(this).html();
      var months = rep(month);
      console.log(year, months, day)//获取到的年月日;
      // 在这里发送ajax请求
      var url = 'http://47.93.242.234:9095/kaiyue/roa/personwork/pwcalendar.do?method=dayCalendar&';
      var urlCan = `year=${year}&month=${months}&day=${day}`;
      window.open(url + urlCan, '', 'height=500, width=600, top=80,left=180, toolbar=no,menubar=no, scrollbars=no, resizable=no,location=no, status=no')
    })
    function rep(month) {
    ...
    var url = "http://47.93.242.234:9095/kaiyue/roa/msgRemind.do?method=listIssueInfo";
    $.get("url")
  });
